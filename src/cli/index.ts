...
program
  ...
  .action(async (options) => {
    try {
      ...
      // 4. Load track-map
      let trackMap = { version: 1 as const } as any;
      if (options.trackMap) {
      ...
      const publishResult = await publishToTrack(
        apiClient,
        payload.material as any, // Cast because questionIds are string[] in toTrackPayloads but numbers are required for the client
      ...
      if (!isDryRun && options.trackMap) {
        ...
        if (!trackMap.questions) trackMap.questions = {};
        if (!trackMap.materials) trackMap.materials = {};
        ...
        trackMap.materials[materialKey] = {
          ...
          question_keys: parsedQti.items.map((item: any) => `${baseKey}/${item.identifier}`),
          updated_at: timestamp,
