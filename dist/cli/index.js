...
program
    ...
    try {
        ...
        const appspace = options.appspace ??
...
function validateTrackMapTargetConflicts(options, target) {
    if (target === undefined)
        return;
    assertTargetMatch(options.baseUrl, '--base-url', target.base_url, 'Track map target base_url', normalizeBaseUrl);
    assertTargetMatch(options.appspace, '--appspace', target.appspace, 'Track map target appspace');
    assertTargetMatch(process.env.TRACK_TCM_BASE_URL, 'TRACK_TCM_BASE_URL', target.base_url, 'Track map target base_url', normalizeBaseUrl);
    assertTargetMatch(process.env.TRACK_TCM_APPSPACE, 'TRACK_TCM_APPSPACE', target.appspace, 'Track map target appspace');
}
function validateTrackMapSessionConflicts(session, target) {
    if (target === undefined)
        return;
    assertTargetMatch(session.baseUrl, 'Track session baseUrl', target.base_url, 'Track map target base_url', normalizeBaseUrl);
    assertTargetMatch(session.appspace, 'Track session appspace', target.appspace, 'Track map target appspace');
}
function assertTargetMatch(value, valueLabel, targetValue, targetLabel, normalize = identity) {
    if (value === undefined)
        return;
    if (normalize(value) !== normalize(targetValue)) {
        throw new Error(`${valueLabel} ${value} conflicts with ${targetLabel} ${targetValue}`);
    }
}
